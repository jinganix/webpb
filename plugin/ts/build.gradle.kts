...
val buildHostNativeBinary by tasks.registering(Copy::class) {
  ...
  from(pluginDir.file("bin/webpb-protoc-ts$pluginExt"))
...
platforms.forEach { (goOs, goArch, resourceName) ->
  tasks.register<Exec>("buildNativeBinary_$resourceName") {
    group = "build"
    description = "Cross-compile webpb-protoc-ts for $resourceName"
    onlyIf {
    ...
    val output =
      nativeResourceDir.map {
        it.file("native/$resourceName/webpb-protoc-ts$ext").asFile
      }
    outputs.file(output)
    doFirst {
...
signAndPublish(artifactId, "The webpb protoc plugin for TS")
